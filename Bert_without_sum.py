...
class Bert_without_sum:

    def __init__(self):

        self.calculate(self, data_cv_withsum, data_cv_withoutsum, job_discr)


    drive.mount('content')

    def calculate(self, data_cv_withsum, data_cv_withoutsum, job_discr):
        with open('okapi_withoutsum.txt', "w", encoding='utf-8') as file:

            for i in range(len(data_cv_withsum)):
                bert_array_sum = (Bert(data_cv_withsum[i], job_discr))
                results_df['bert'] = bert_array_sum[0]
                ...
                file.write(doc.loc[doc['processed_text'] == data_cv_withsum[i]].values[0][0])
                file.write("\n ")
                ...
                bert_array = (Bert(data_cv_withoutsum, job_discr))
                results_df['bert_withoutsum'] = bert_array[0]
                ranking1 = results_df.sort_values('bert_withoutsum', ascending=False)
                ...
                first_line = ('\n 1st place in bert = ' + rank_1 + " " + score_1 + ' in Bert without summary')
                ...
                second_line = ('\n 3rd bert in bert:' + rank_3 + " " + score_3 + ' in Bert without summary')
                ...
                third_line = ('\n 5th place in bert:' + rank_5 + " " + score_5 + ' in Bert without summary')
                ...
                fourth_line = ('\n middle place in bert:' + rank_middle + " " + score_middle + ' in Bert without summary')
                ...
                fifth_line = ('\n last place in bert:' + rank_last + " " + score_last + ' in Bert without summary')
