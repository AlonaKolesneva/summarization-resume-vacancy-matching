...
import numpy
class Ngrams_words_without_sum:
    ...
    def calculate(self, data_cv, job_discr):
        with open('ngrams(word)_withoutsummary.txt', "w", encoding='utf-8') as file:
            for i in range(len(data_cv)):
                ...
                results_df_n_grams_word['n-grams'] = n_grams_array
                ranking1 = results_df_n_grams_word.sort_values('n-grams', ascending=False)
                ...
                first_line = ('\n 1st place in bert = ' + rank_1 + " " + score_1 + ' in n-grams(word) without summary')
                ...
                second_line = ('\n 3rd bert in bert:' + rank_3 + " " + score_3 + ' in n-grams(word) without summary')
                ...
                third_line = ('\n 5th place in bert:' + rank_5 + " " + score_5 + ' in n-grams(word) without summary')
                ...
                fourth_line = ('\n middle place in bert:' + rank_middle + " " + score_middle + ' in n-grams(word) without summary')
                ...
                fifth_line = ('\n last place in bert:' + rank_last + " " + score_last + ' in n-grams(word) without summary')
