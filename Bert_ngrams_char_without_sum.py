...
class Bert_ngrams_char_without_sum:
    ...
    def calculate(self, data_cv, job_discr):
        with open('Bert_ngrams_char_withoutsummary.txt', "w", encoding='utf-8') as file:
            for i in range(len(data_cv)):
                ...
                results_df_bert_n_grams_char['bert-n-grams'] = Bert_ngrams_matrix_sum
                ranking1 = results_df_n_grams_char.sort_values('bert-n-grams', ascending=False)
                ...
                first_line = ('\n 1st place in bert = ' + rank_1 + " " + score_1 + ' in Bert + ngrams(char) without summary')
                ...
                second_line = ('\n 3rd bert in bert:' + rank_3 + " " + score_3 + ' in Bert + ngrams(char) without summary')
                ...
                third_line = ('\n 5th place in bert:' + rank_5 + " " + score_5 + ' in Bert + ngrams(char) without summary')
                ...
                fourth_line = ('\n middle place in bert:' + rank_middle + " " + score_middle + ' in Bert + ngrams(char) without summary')
                ...
                fifth_line = ('\n last place in bert:' + rank_last + " " + score_last + ' in Bert + ngrams(char) without summary')
