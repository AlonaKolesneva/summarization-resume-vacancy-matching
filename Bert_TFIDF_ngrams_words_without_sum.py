...
class Bert_TF_IDF_ngrams_words_without_sum:
    ...
    def calculate(self, data_cv, job_discr):
        with open('Bert_TFIDF_ngrams_words_withoutsummary.txt', "w", encoding='utf-8') as file:
            for i in range(len(data_cv)):
                ...
                first_line = ('\n 1st place in bert = ' + rank_1 + " " + score_1 + ' in Bert TF-IDF + ngrams(words) without summary')
                ...
                second_line = ('\n 3rd bert in bert:' + rank_3 + " " + score_3 + ' in Bert + TF-IDF + ngrams(words) without summary')
                ...
                third_line = ('\n 5th place in bert:' + rank_5 + " " + score_5 + ' in Bert + TF-IDF + ngrams(words) without summary')
                ...
                fourth_line = ('\n middle place in bert:' + rank_middle + " " + score_middle + ' in Bert + TF-IDF + ngrams(words) without summary')
                ...
                fifth_line = ('\n last place in bert:' + rank_last + " " + score_last + ' in Bert + TF-IDF + ngrams(words) without summary')
